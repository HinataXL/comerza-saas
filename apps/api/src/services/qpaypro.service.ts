...
interface PaymentLinkPayload {
  ...
  items: { name: string; id: string; quantity: number; price: number }[];
}

export const createPaymentLink = async (credentials: QPayProCredentials, payload: PaymentLinkPayload): Promise<string> => {
