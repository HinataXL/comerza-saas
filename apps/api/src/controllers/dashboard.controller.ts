...
export const getDashboardMetrics = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const paymentMethodsData = paymentMethodsStats.map(pm => {
      let color = '#4b5563';
      if (pm.paymentMethod === 'Tarjeta') color = '#2563eb';
      else if (pm.paymentMethod === 'Transferencia') color = '#14b8a6';
      else if (pm.paymentMethod === 'Efectivo') color = '#eab308';
      else if (pm.paymentMethod === 'Link de pago') color = '#8b5cf6';
    ...
    felStatusCounts.forEach(f => {
      ...
      if (f.felStatus === 'PENDIENTE') pendientes = f._count;
