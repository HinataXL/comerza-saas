...
const handleWhatsAppWebhook = async (req, res) => {
    try {
        ...
        const recentReservation = await prisma_1.prisma.reservation.findFirst({
            where: {
                status: 'PENDING',
                customer: {
                    phone: {
        ...
        logger_service_1.logger.info(`Reservación ${recentReservation.id} actualizada a ${actionStr} exitosamente vía Twilio.`);
    ...
    `);
        ...
        logger_service_1.logger.error('Error handling Twilio WhatsApp webhook:', error);
