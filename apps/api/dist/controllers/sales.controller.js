...
exports.getSaleById = exports.updateSaleStatus = exports.createQuickCharge = exports.getSales = exports.createSale = void 0;
const prisma_1 = require("../lib/prisma");
const qpaypro_service_1 = require("../services/qpaypro.service");
const recurrente_service_1 = require("../services/recurrente.service");
const createSale = async (req, res) => {
    try {
        ...
        if (customerId) {
            customerDetails = await prisma_1.prisma.customer.findUnique({
        ...
        if (paymentMethod === 'Link de pago') {
            try {
                paymentLink = await (0, qpaypro_service_1.createPaymentLink)({
            ...
            try {
                await (0, recurrente_service_1.createTerminalSessionCommand)({
...
const createQuickCharge = async (req, res) => {
    try {
        const tenantId = req.user?.tenantId;
        const userId = req.user?.id;
        ...
        if (paymentMethod === 'Recurrente NFC') {
            if (!tenant.recurrenteSecretKey || !tenant.recurrenteTerminalId) {
                await prisma_1.prisma.sale.delete({ where: { id: sale.id } });
        ...
        try {
            const paymentLink = await (0, qpaypro_service_1.createPaymentLink)({
                apiKey: tenant.qpayproApiKey || '',
                apiSecret: tenant.qpayproApiSecret || ''
            }, {
                amount: Number(amount),
...
exports.createQuickCharge = createQuickCharge;
const updateSaleStatus = async (req, res) => {
    try {
        ...
        const updatedSale = await prisma_1.prisma.sale.update({
            where: { id: id },
            data: { status }
        });
